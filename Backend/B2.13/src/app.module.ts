...
@Module({
  ...
  providers: [
    ...
    // AuditInterceptor runs after Logging/Security so
    // request.headers['x-correlation-id'] (set by LoggingInterceptor) and
    // any security-layer context are already established by the time it
    // reads them via AuditContextResolver — see
