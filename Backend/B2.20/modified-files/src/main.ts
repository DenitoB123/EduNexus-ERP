...
// B2.20 addition — see IMPLEMENTATION_SUMMARY_B2_20.md.
import { BootstrapDiagnosticsService } from './common/bootstrap/bootstrap-diagnostics.service';
...
async function bootstrap(): Promise<void> {
  ...
  // B2.20: runs configuration cross-field validation + startup
  // connectivity verification + logs platform metadata, in one place,
  // before the app starts accepting traffic. Throws (aborting boot)
  // only on a hard configuration error; a down critical dependency is
  // logged loudly but does not prevent boot — see
  // BootstrapDiagnosticsService's docstring for why, and /health/ready
  // for ongoing readiness after boot.
  const bootstrapDiagnostics = app.get(BootstrapDiagnosticsService);
  await bootstrapDiagnostics.runAndLog();
